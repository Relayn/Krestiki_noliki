...
class TicTacToe:
    ...
    def __init__(self):
        ...
        self.score_O = 0  # Счётчик побед игрока 0
    ...
    def create_score_label(self):
        ...
        self.score_label = tk.Label(
            self.window,
            text=f"X: {self.score_X} | 0: {self.score_O}",
            font=self.SCORE_FONT,
            bg=self.BG_COLOR,
            fg=self.TEXT_COLOR
        )
    ...
    def _is_winning_line(self, a, b, c):
        """Проверяет, что три значения равны и непустые."""
        return a == b == c and a != ""
    ...
    def check_winner(self):
        ...
        """
        for i in range(3):
            if self._is_winning_line(self.buttons[i][0]["text"],
                                     self.buttons[i][1]["text"],
                                     self.buttons[i][2]["text"]):
                return True
            if self._is_winning_line(self.buttons[0][i]["text"],
                                     self.buttons[1][i]["text"],
                                     self.buttons[2][i]["text"]):
                return True
        if self._is_winning_line(self.buttons[0][0]["text"],
                                 self.buttons[1][1]["text"],
                                 self.buttons[2][2]["text"]):
            return True
        if self._is_winning_line(self.buttons[0][2]["text"],
                                 self.buttons[1][1]["text"],
    ...
    def handle_win(self):
        ...
        if self.current_player == "X":
            ...
            self.score_O += 1
        self.score_label.config(text=f"X: {self.score_X} | 0: {self.score_O}")
        self.highlight_buttons(self.WIN_COLOR)
        messagebox.showinfo("Игра окончена", f"Игрок {self.current_player} победил!")
        if self.score_X == 3 or self.score_O == 3:
    ...
    def handle_draw(self):
        """Обрабатывает ничью."""
        self.highlight_buttons(self.DRAW_COLOR)
